function editMoving(){ //Fixed Platform
    if (win.closed){
        window.win = window.open("", "_blank", "title=Configuration Window");
    }
    win.focus()
    win.document.title = "New Moving Platform";
...
<div id="myDIV" class="header">
  <h2 style="margin:5px">Moving Platforms</h2>
  <input type="text" id="myInput" placeholder="starting_x,starting_y,width,height,ending_x,ending_y">
...
function executeScriptElements(containerElement) {
  ...
  Array.from(scriptElements).forEach((scriptElement) => {
        ...
        */
        ...
        win.document.querySelectorAll("ul li").forEach((el) => {
            ...
            if (a.length == 6){
                ...
                if (c){
                    moving = moving.concat(d);
                    movingpos.push(d[0]);
                    movingpos.push(d[1]);
                    movingtick = Array(parseInt(moving.length/6)).fill(0);
                    movingdir = Array(parseInt(moving.length/6)).fill(1);
                    //win.alert("append");
                }
                else{
    ...
    var l = parseInt(moving.length/6);
    var p = [...moving];
    for (let index = 0; index < l; index++) {
        var item = index * 6;
        win.document.querySelector("#myInput").value = p.slice(item, item + 6).toString();
